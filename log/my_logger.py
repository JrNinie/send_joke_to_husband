...
LOGGER_PATH = settings.LOGGER_PATH
...
class MyLogFormatter(logging.Formatter):
    ...
    complet_fmt = "[%(asctime)s] [%(levelname)s] [%(module)s/%(filename)s :%(lineno)s %(funcName)s()] - %(message)s"
...
def get_logger(logger_name=__name__, level=LOGGER_LEVEL):
    ...
    if not logger.handlers:  # logger existance check (avoid to add other handlers to the same logger)
        # 1 create handlers
